...
| Production and inventory | Partial | Production plans, equipment, additives, packaging, inventory, transfers, purchases, shopping, and elapsed lifecycle timing | Exact results require complete movement, sale, revenue, and cost inputs |
| Property blueprints | Partial | Placement, construction order, cost, collision, access, temperature, lighting, capacity, schedules, employee logistics, route candidates | Mutable runtime state, dynamic obstacles, and unsupported collider proof remain outside the static model |
...
Exporter `0.0.21` targets *Schedule I* `0.4.6f13`, MelonLoader `0.7.3`, S1API `3.1.6`, and AssetRipper `1.3.14`.
